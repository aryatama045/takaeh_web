...
class Email extends Admin_Controller
{
    ...
    function index()
    {
        $mailbox = new PhpImap\Mailbox(
            ...
            , __DIR__, 'UTF-8');
        try {
            $this->mailsIds = $mailbox->searchMailbox('ALL');
        } catch(PhpImap\Exceptions\ConnectionException $ex) {
            echo "IMAP connection failed: " . $ex;
            die();
        }

        $data['arrFolders'] = $mailbox->getListingFolders();
        $mails = $mailbox->getMailsInfo($this->mailsIds);
        $records = [];
        for ($i=0; $i < sizeof($mails); $i++) {
            ...
            if($mailbox->getMail($this->mailsIds[$i])->hasAttachments()) {
                $record['attachment'] = 1;
            } else {
                $record['attachment'] = 0;
            }
