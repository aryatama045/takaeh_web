...
class Menu extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->template_admin('menu/index', $data);
    }
    ...
    public function menu($group_id)
    {
        ...
        $data['group_title'] = $this->manager->get_menu_group_title($group_id);
        $data['menu_groups'] = $this->manager->get_menu_groups();
        $this->template_admin('menu/edit', $data);
    }
    ...
    public function edit($id)
    {
        ...
        $data['menu_groups'] = $this->manager->get_menu_groups();
        $this->load->view('menu/modal_edit', $data);
    }
    ...
    public function save()
    {
        ...
        if ($title) {
            ...
            if (!empty($data['title'])) {
                ...
                if ($this->db->update('menus', $data, 'id' . ' = ' . $data['id'])) {
                    if ($item_moved) {
                        //move sub items
                        $ids = $this->manager->get_descendants($data['id']);
                        if (!empty($ids)) {
                            $sql = sprintf('UPDATE %s SET %s = %s WHERE %s IN (%s)', 'menus', 'group_id', $group_id, 'id', $ids);
                        ...
                        $d['title'] = $data['title'];
                        $d['url'] = $data['url'];
                        $d['icon'] = $data['icon']; //klass instead of class because of an error in js
                        $response['menu'] = $d;
                    }
                } else {
                    $response['status'] = 2;
            ...
            if (!empty($menu2)) {
                foreach ($menu2 as $k => $v) {
                    ...
                    foreach ($v as $v2) {
                        $data['parent_id'] = $k;
                        $data['position'] = $i;
                        if ($this->db->update('menus', $data, 'id' . ' = ' . $v2)) {
                            $success++;
                        }
                        $i++;
                    }
    ...
    private function get_label($row)
    {
        $label = '<div class="ns-row">' .
            ...
            '<span class="fa fa-edit" style="color: #444"></span> ' .
            '</a>' .
